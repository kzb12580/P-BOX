...
// ==================== 协议开关配置 ====================
let ev = ENABLED_PROTOCOLS.includes('vless');      // VLESS 协议
let et = ENABLED_PROTOCOLS.includes('trojan');     // Trojan 协议  
let ex = false;  // xhttp (暂不支持)
let tp = '';     // Trojan 密码（从 KV 读取）

// ==================== 默认配置 ====================
let at = '';  // 用户 UUID（从 KV 动态验证）
let fallbackAddress = '';
...
// 优选 IP 自动更新配置
const PROXYIP_SOURCE_URL = 'https://raw.githubusercontent.com/byJoey/cfnew/refs/heads/main/%E6%98%8E%E6%96%87%E6%BA%90%E5%90%97';
let lastProxyIPUpdate = 0;
const PROXYIP_UPDATE_INTERVAL = 3600000; // 1小时更新一次
...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			// 🔄 自动更新优选 IP 列表（每小时检查一次）
			ctx.waitUntil(updateProxyIPsFromSource());
			
			// 使用部署时设置的用户凭证
			at = (env.u || env.U || '{{USER_ID}}').toLowerCase();
			const subPath = (env.d || env.D || at).toLowerCase();
			tp = env.tp || env.TP || '{{TROJAN_PASSWORD}}';
            ...
            <div id="configContent" style="display: none;">
                ...
                <form id="otherConfigForm" style="margin-bottom: 20px;">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="text" id="preferredIPsURL" placeholder="留空使用内置 Cloudflare 官方 IP（推荐）" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00ff00; color: #00ff00; font-family: 'Courier New', monospace; font-size: 14px;">
                        <small style="color: #00aa00; font-size: 0.85rem;">自定义优选IP列表来源URL，留空则使用内置官方IP段（性能稳定）</small>
                    </div>
                    <div style="margin-bottom: 15px;">
...
async function fetchAndParseNewIPs() {
    // 如果没有配置外部优选 IP 源，直接返回空数组（使用内置 backupIPs）
    if (!piu || piu === '') {
        return [];
    }
...
async function handleConfigAPI(request) {
    if (request.method === 'GET') {
        ...
        try {
            ...
            const newPreferredIPsURL = getConfigValue('yxURL', '') || '';
            // 如果用户没有自定义优选 IP 源，使用内置配置
            if (newPreferredIPsURL === '' || !newPreferredIPsURL) {
                backupIPs = [
                    { domain: 'ProxyIP.US.CMLiussss.net', region: 'US', regionCode: 'US', port: 443 },
                    { domain: 'ProxyIP.SG.CMLiussss.net', region: 'SG', regionCode: 'SG', port: 443 },
                    { domain: 'ProxyIP.JP.CMLiussss.net', region: 'JP', regionCode: 'JP', port: 443 },
...
// 从源 URL 自动更新优选 IP 列表
async function updateProxyIPsFromSource() {
    const now = Date.now();
    
    // 检查是否需要更新（距离上次更新超过1小时）
    if (lastProxyIPUpdate && (now - lastProxyIPUpdate < PROXYIP_UPDATE_INTERVAL)) {
        return;
    }
    
    try {
        console.log('🔄 正在从 GitHub 获取最新优选 IP 列表...');
        const response = await fetch(PROXYIP_SOURCE_URL, {
            headers: {
        ...
        if (!response.ok) {
            console.error('❌ 获取优选 IP 失败:', response.status);
            return;
        }
        ...
        // 提取 backupIPs 数组
        const backupIPsMatch = sourceCode.match(/let backupIPs\s*=\s*(\[[\s\S]*?\]);/);
        if (backupIPsMatch) {
            try {
                // 安全地解析 backupIPs
                const newBackupIPs = eval('(' + backupIPsMatch[1] + ')');
                if (Array.isArray(newBackupIPs) && newBackupIPs.length > 0) {
                    backupIPs = newBackupIPs;
                    console.log('✅ 成功更新 backupIPs，共', backupIPs.length, '个节点');
                }
            } catch (e) {
                console.error('❌ 解析 backupIPs 失败:', e.message);
            }
        }
        
        // 提取 directDomains 数组
        const directDomainsMatch = sourceCode.match(/const directDomains\s*=\s*(\[[\s\S]*?\]);/);
        if (directDomainsMatch) {
            try {
                const newDirectDomains = eval('(' + directDomainsMatch[1] + ')');
                if (Array.isArray(newDirectDomains) && newDirectDomains.length > 0) {
                    directDomains.length = 0; // 清空原数组
                    directDomains.push(...newDirectDomains);
                    console.log('✅ 成功更新 directDomains，共', directDomains.length, '个域名');
                }
            } catch (e) {
                console.error('❌ 解析 directDomains 失败:', e.message);
            }
        }
        
        lastProxyIPUpdate = now;
        console.log('✅ 优选 IP 列表更新完成');
        
    } catch (error) {
        console.error('❌ 更新优选 IP 列表失败:', error.message);
    }
}
